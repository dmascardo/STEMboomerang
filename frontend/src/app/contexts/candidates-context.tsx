...
interface CandidatesContextType {
  ...
  updateCandidate: (id: string, updates: Partial<CandidateRecord>) => void;
...
export function CandidatesProvider({ children }: { children: ReactNode }) {
  ...
  const recalculateMissingFields = (record: CandidateRecord): CandidateRecord => {
    ...
    return {
      ...record,
      required_fields_missing: missingRequired.length > 0 ? missingRequired : null,
  ...
  const updateCandidate = (id: string, updates: Partial<CandidateRecord>) => {
    setCandidates(prev =>
      prev.map(c => {
        if (c.id !== id) return c;
